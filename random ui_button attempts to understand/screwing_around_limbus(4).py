...
class SpriteManager:
    ...
    def load_all_sprites(self):
        """Load all available sprites from the sprites directory"""
        characters = ["yi_sang", "faust", "don_quixote"]
        ...
        for char in characters:
            ...
            if os.path.exists(idle_path):
                try:
                    ...
                    sprite_set.idle = pygame.transform.scale(sprite_set.idle, (80, 100))
                except pygame.error:
            ...
            if os.path.exists(attack_path):
                try:
                    ...
                    # Scale to reasonable size
                    sprite_set.attack = pygame.transform.scale(sprite_set.attack, (80, 100))
                except pygame.error:
    ...
    def save_default_sprites(self):
        """Save default sprite templates that users can modify"""
        characters = [
            ("yi_sang", (100, 100, 150)),
            ("faust", (120, 80, 150)), 
            ("don_quixote", (200, 150, 50))
        ]
        
        for char_name, color in characters:
            char_dir = os.path.join(self.sprites_dir, char_name)
            
            # Create idle sprite template
            idle_surface = pygame.Surface((80, 100), pygame.SRCALPHA)
            pygame.draw.ellipse(idle_surface, color, (20, 20, 40, 60))
            pygame.draw.circle(idle_surface, (255, 220, 177), (40, 30), 15)
            
            idle_path = os.path.join(char_dir, "idle_template.png")
            pygame.image.save(idle_surface, idle_path)
            
            # Create attack sprite template
            attack_surface = pygame.Surface((80, 100), pygame.SRCALPHA)
            pygame.draw.ellipse(attack_surface, color, (15, 25, 40, 60))
            pygame.draw.circle(attack_surface, (255, 220, 177), (35, 35), 15)
            # Add attack effect
            pygame.draw.line(attack_surface, (255, 255, 255), (55, 40), (75, 30), 3)
            
            attack_path = os.path.join(char_dir, "attack_template.png")
            pygame.image.save(attack_surface, attack_path)
        
        print(f"Default sprite templates saved to {self.sprites_dir}/")
        print("Rename 'idle_template.png' to 'idle.png' and 'attack_template.png' to 'attack.png' to use them!")
...
class Enemy:
    ...
    def draw_sprite_manager(self):
        ...
        for i, (display_name, char_key) in enumerate(characters):
            ...
            # Show idle sprite or placeholder
            ...
            # Show attack sprite or placeholder
        ...
        instructions = [
            ...
            "• Recommended size: 80x100 pixels or similar ratio",
            "",
    ...
    def draw_sinner(self, sinner):
        ...
        # Health bar
        self.draw_health_bar(x - 50, y + 70, sinner.current_hp, sinner.identity.max_hp)
        
        # Highlight if selected
        if sinner == self.selected_sinner:
            pygame.draw.circle(self.screen, YELLOW, (x, y), 50, 4)
        
        # Add attack effect for non-sprite characters
        if not sprite and sinner.is_attacking and not sinner.is_returning:
            for i in range(3):
                offset = i * 10
