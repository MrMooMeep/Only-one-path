...
class LimbusCompany:
    ...
    def draw_sinner(self, sinner):
        ...
        if len(name_parts) > 1:
            ...
            identity_rect = identity_text.get_rect(center=(x, y - 40))
            self.screen.blit(identity_text, identity_rect)
        
        # Health bar
        self.draw_health_bar(x - 50, y + 40, sinner.current_hp, sinner.identity.max_hp)
        
        # Highlight if selected
        if sinner == self.selected_sinner:
            pygame.draw.circle(self.screen, YELLOW, (x, y), 35, 4)
    ...
    def draw_enemy(self, enemy):
        ...
        pygame.draw.rect(self.screen, WHITE, (x - 40, y - 40, 80, 80), 3)
    ...
    def draw_skill_selection(self):
        ...
        for i, skill in enumerate(self.selected_sinner.identity.skills):
            ...
            self.screen.blit(num_text, (skill_rect.x + 300, skill_rect.y + 25))
    ...
    def draw_combat(self):
        ...
        # If a sinner is selected and it's their turn, show skill selection
        if (self.selected_sinner and 
            self.turn_queue and 
    ...
    def execute_skill(self, attacker, skill, target):
        ...
        for _ in range(skill.coin_count):
            if random.random() > 0.5:  # Heads
                total_damage += random.randint(3, 8)
        
        # Apply damage
        target.take_damage(total_damage)
        
        # Visual feedback (you could add animations here)
        print(f"{attacker.identity.name if isinstance(attacker, Sinner) else attacker.name} used {skill.name} for {total_damage} damage!")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.state == GameState.MENU:
                    ...
                    # Use skill (if it's the selected sinner's turn)
